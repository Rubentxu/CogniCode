...
| **PR8 Analytics Runtime Wiring** | `fix/analytics-registry-wiring` | ✅ Merged | v0.81.2 | [#219](https://github.com/Rubentxu/CogniCode/pull/219) |
...
Session Handover 2026-07-29: E28.4 PR6 closes the wiring blockers. Remaining E28.4 work: wire AlgorithmRegistry from runtime composition root with algorithm descriptors admitted at startup. — **CORRECTION (2026-08-04)**: D4 audit found AlgorithmRegistry still not wired to MCP handler (zero call sites). PR #219 (v0.81.2, fix/analytics-registry-wiring) adds missing `with_analytics()` call in `bootstrap_with_backend`, wires both fields through `into_mcp_handler`, and adds `InMemoryLineageStore` as ephemeral fallback.
...
| `e19-6-viewsrs-split` | Extract narrative executors from views.rs god-class (9460 LOC) to `domain/narrative/` (~837 LOC) | ADR-002 | MEDIUM | DONE | [#218](https://github.com/Rubentxu/CogniCode/pull/218) | v0.81.1 |
...
| 3 | `e14-narrative-runtime` | MAJOR | `cognicode-explorer`, `cognicode-core` | Implement `ProjectDiary`, `ComposedNarrative`, and `ExampleObject` as runtime artifacts. **Cycle 1 DONE (v0.83.0)** — EmbedResolver + wiring en 4 narrative shapers; Cycle 2 (LadybugDB persistence) pending |
...
| `e29-0-define-new-ports` | Define 8 port traits in `domain/ports/` (5 net-new or relocated: `RevisionStore`, `FederationStore`, `IngestCommit`, `QualityStore`, `ViewSpecStore`; 3 existing: `ManifestStore`, `SessionStore`, `ReportStore`) per ADR-028 | None | 8 trait modules in `domain/ports/` with domain error types (5 net-new or relocated: `RevisionStore`, `FederationStore`, `IngestCommit`, `QualityStore`, `ViewSpecStore`; 3 existing: `ManifestStore`, `SessionStore`, `ReportStore`); all in `cognicode_core`; obsolete `ViewSpecRepository` row above | **DONE** (SDDK archived 2026-08-01, release v0.76.7, branch feat/e29-0-define-new-ports stacked chain; PR #179 PR1 + PR #180 PR2 + PR #181 PR3; scope B Pragmatic — unified `QualityRepository`+`QualityWritePort` into `QualityStore`, used wire-format `ViewSpecPayload` bridge for `ViewSpecStore` to avoid 1457-line DTO move; 1 W1 cosmetic: `IngestCommit` is Phase 0 placeholder; full verdict in `sddk/e29-0-define-new-ports/verify-report.md`) |
| `e29-0-refactor-call-sites` | Migrate remaining direct `PostgresRepository` consumers behind `Arc<dyn Port>` handles (1 new port `CallGraphStore` + 2 production call sites migrated) | e29-0-define-new-ports | 2 production call sites (pg_graph_executor.rs + postgres_bridge.rs) reach `Arc<dyn CallGraphStore>` instead of `PostgresRepository` direct; runtime composition root has the new `call_graph_store` field; `with_pool` accessor preserved (D1); tests still green (3385 passed) | **DONE** (SDDK archived 2026-08-01, release v0.76.8, branch feat/e29-0-refactor-call-sites, PR #182 + trunk commit `b01671f6`; explore report found 2 production + ~14 test fixture call sites — net scope smaller than the original '~30' framing; new port `CallGraphStore` adds to the 8-trait surface from `e29-0-define-new-ports` → 9 traits total ready for Phase 1 LadybugStore; **reconciled** to ADR-028 §3 actual method signatures per audit `sddk/audit-e29-0/code-vs-design-audit.md` at `c3e12ab4`/`b01671f6`; openspec specs reconciled) |
...
| `e29-6-ladybug-store-wiring` | Wire all 10 LadybugStore port traits into runtime composition root via `bootstrap_ladybug()`; fix `LadybugStore::open()` schema init bug (4 schemas); add `--db <PATH>` CLI flag to api+mcp binaries | e29-1-ladybug-adapter | All 10 ports available after bootstrap; DB file created automatically; schema migrations idempotent; `--db` flag honored | **DONE** (PR #221 merged, v0.82.0; 2 commits; A-min path; verify PASS_WITH_WARNINGS; debt-verify PASS_WITH_WARNINGS; bootstrap_with_backend async decoration is decorative (0 .await) — follow-up SDDK recommended) |
...
E29 Phase 3  (e29-3-* + debt-*)   (DONE en v0.80.0 + v0.80.1)
        ↓
E29 Phase 4  (e29-6-*)            (DONE en v0.82.0 — wire LadybugStore into runtime)
        ↓

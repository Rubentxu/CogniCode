...
Last updated: 2026-08-02 (v0.76.1: pre-existing debt batch — tokio Handle::current panic fix + cycle_dominators assertion + PlanHash helpers; v0.76.0: E12 ViewKind Realization. E29 S5 latency spike archived. E29 S6 cypher-compat spike archived — spike gate COMPLETE. E29 Phase 0 e29-0-clean-ports archived — port surface DB-agnostic. E29 Phase 0 e29-0-move-view-specrepo marked OBSOLETE — trait deleted 2026-07-30, ROADMAP row stale. E29 Phase 0 e29-0-define-new-ports archived — 8 ports cover ADR-028 surface. E29 Phase 0 e29-0-refactor-call-sites archived — 9th port CallGraphStore added, remaining direct PG call sites migrated. E29 Phase 0 reconciliation — ADR-028 §3 reconciled to actual method signatures, 3 Phase 0 stubs (ManifestStore.delete_manifest_entry, ReportStore.save_report, IngestCommit.commit_revision) implemented with real SQL/tx at `b01671f6`/`c3e12ab4`, openspec specs reconciled. E29 Phase 1 starter — `cognicode-ladybug` crate skeleton shipped with 7 default ports + `multimodal-9ports` feature gate. E29 Phase 1 Priority 1 — `ManifestStore` per-port lbug-side SQL shipped (3 methods, 5 integration tests, real lbug db round-trip); upsert via read-then-conditional-write due to lbug 0.19 single-column-PK constraint. E29 Phase 1 Priority 2 — `SessionStore` per-port lbug-side SQL shipped (3 methods save/load/list, 5 integration tests including scope-isolation security check); natural single-column STRING PK on `id` so no read-then-conditional-write workaround needed.)

...
| PRF-F2-W5-001 | Lookup `name → SymbolId` con resolución scope-aware: 1 candidato → ese; múltiples en archivo del caller → local; múltiples en archivos distintos del mismo crate root → ese; ambigüedad genuina → descarta HONESTAMENTE la arista (no inventa homónimos). Cross-file edges preservados en ambos paths (per_file y full). | `crates/cognicode-core/src/infrastructure/graph/per_file_graph.rs` — `GlobalSymbolIndex` (nuevo), `BuildFileResult`/`CrossFileEdge` (alias), `build_global_index`, refactor de `merge_with_report` y `build_file_graph` para diferir aristas cross-file; `crates/cognicode-core/src/infrastructure/graph/strategy.rs` — `FullGraphStrategy::build_full_graph` reescrito a dos pasadas | `w5_cross_file_call_edge_resolves_to_correct_symbol`, `w5_cross_file_call_edge_also_present_in_full`, `w5_intra_file_duplicate_does_not_invent_cross_file_edges`, `w5_compute_overload_no_call_site_yields_no_invented_edges`; `global_index_tests` × 4 (reglas del resolver a nivel unitario) | `evidence/CERTIFICATES.md` (PRF-F2-W5, pendiente de generación); H-R4-2 cerrado en `docs/prf/JOURNAL.md` §14 | `3f27a31d` |
| PRF-F2-W7-001 | Integrar la resolución scope-aware de F2.W5 (H-R4-2) en el camino real del binario (`AnalysisService::build_project_graph`). El binario no invocaba `FullGraphStrategy`/`PerFileStrategy`; tenía un `HashMap<String, SymbolId>` propio con tie-break FQN lexicográfico que violaba D33 (inventaba destinos contra homónimos). | `crates/cognicode-core/src/application/services/analysis_service.rs::build_project_graph` reescrito para usar `GlobalSymbolIndex` (mismo tipo que F2.W5); `all_relationships` ahora `(caller_fqn, caller_file, callee_name)`; `crates/cognicode-core/src/infrastructure/graph/mod.rs` promueve `per_file_graph` a `pub mod` | `w7_scope_aware_resolution_tests` (6 tests): `w7_single_candidate_cross_file_resolves_to_nested_callee`, `w7_homonym_in_callers_file_resolves_locally`, `w7_single_candidate_cross_file_resolves_to_ambig_compute`, `w7_homonym_three_way_resolves_to_callers_file_local`, `w7_two_way_homonym_no_caller_file_honest_drop` (D33: ambigüedad genuina → None), `w7_no_invented_edges_outside_crate_root` | `evidence/CERTIFICATES.md` (PRF-F2-W7, pendiente de generación); H-R4-2 cerrado en `docs/prf/JOURNAL.md` §15 | `5ce8eb1e` |
| PRF-F2-W8-001 | Errores silenciosos en `AnalysisService::build_project_graph`: las 4 fuentes (`read_to_string(...).ok()?`, `TreeSitterParser::with_cache(...).ok()?`, `find_all_symbols_with_path(...).unwrap_or_default()`, `find_call_relationships(...).unwrap_or_default()`) se reemplazan por captura explícita + clasificación (`SkipReason::{Read, Parse, UnsupportedExtension, Other}`) + push a `Vec<SkippedFile>`. API pública `AnalysisService::get_last_build_report() -> Option<BuildReport>` enumera los omitidos. Surface MCP: `handle_build_graph` retorna `skipped_files: Option<Vec<SkippedFileDto>>` (con `path`, `reason_kind`, `reason`). | `crates/cognicode-core/src/application/services/analysis_service.rs` — campo `last_build_report: Mutex<Option<BuildReport>>`; closure de `filter_map` con `match` explícito sobre `io::Error` y `ParseError`; helpers `classify_io` y `classify_parse`; `BuildReport`/`SkippedFile`/`SkipReason` reusados de `infrastructure/graph/per_file_graph.rs` (F2.W2). `crates/cognicode-core/src/interface/mcp/handlers/mod.rs` — campo `skipped_files: Option<Vec<SkippedFileDto>>` en `BuildGraphOutput` + mapeo desde `BuildReport`. | `w8_silent_errors_tests` (3 tests, RED→GREEN): `w8_unreadable_file_is_silently_dropped` (corpus con `chmod 000`, espera `coverage.parsed_files == 1`), `w8_invalid_utf8_file_is_silently_dropped` (espera `coverage_percent < 100%` con 2/3 corruptos), `w8_build_report_enumerates_skipped_files` (pinea API + semántica `Complete`). Suite total: `2118 passed; 0 failed; 27 ignored`. UAT real con binario fresh `/var/home/rubentxu/cargo-targets/release/cognicode-mcp` sobre `/tmp/prf-uat-w8/` (3 archivos: ok + chmod 000 + bytes inválidos) confirma enumeración JSON-RPC de los 2 archivos omitidos. | `evidence/CERTIFICATES.md` (PRF-F2-W8, pendiente de generación); H-R3 cerrado en `build_project_graph` documentado en `docs/prf/JOURNAL.md` §16 | próximo |
| PRF-F2-W9-001 | `file_cache` de `AnalysisService` servía entradas obsoletas cuando los bytes cambiaban preservando el mtime (clave solo mtime). Cache pasa a `(mtime, size, symbols, relationships)`; el cache-hit exige coincidencia de ambos. Actualizados los 3 caminos (`build_project_graph`, `build_graph_per_file`, async) con walk que emite `size`. Limitación documentada: mismo tamaño + mismo mtime sigue indetectado (requeriría hash de contenido). | `crates/cognicode-core/src/application/services/analysis_service.rs` — tipo del campo `file_cache`, 3 bloques cache-hit, 3 inserts, 3 walk maps. | `w9_mtime_tests::w9_content_change_with_preserved_mtime_invalidates_cache` (RED→GREEN). Suite: `2119 passed; 0 failed; 27 ignored`. UAT real: binario release rebuilt sobre `/tmp/prf-uat-w9/`, build #2 (proceso nuevo, mtime restaurado vía `os.utime`) muestra `renamed_function` y no `original_function`. | `evidence/CERTIFICATES.md` (PRF-F2-W9, pendiente); `docs/prf/JOURNAL.md` §17 | `2a121aec` |
| PRF-F2-W10-001 | Equivalencia de aristas y reproducibilidad full ↔ per_file sobre corpus determinista (post W5/W7): mismo conjunto de aristas (caller fqn → callee fqn) en ambas estrategias; conjunto no vacío (anti-regresión H-R4-1); builds consecutivos reproducibles. Caracterización pineada, sin cambio de producción. | `crates/cognicode-core/src/infrastructure/graph/strategy.rs` — `mod w10_equivalence_tests` (3 tests + helpers `symbol_fqns`/`edge_set`). | `w10_full_and_per_file_agree_on_edge_set`, `w10_edge_set_is_non_empty_on_cross_file_corpus`, `w10_repeated_builds_are_reproducible`; probe de sanidad confirmó 1 arista real (`lib.rs:caller:16 → nested/mod.rs:callee:11`) en ambas estrategias. Suite: `2122 passed; 0 failed; 27 ignored`. | `evidence/CERTIFICATES.md` (PRF-F2-W10, pendiente); `docs/prf/JOURNAL.md` §18 | `dc189d54` |
...
| H-F3-1: `find_symbol_usages` (tool MCP `find_usages`) originalmente implementaba walk+parser inline en el handler; sin correspondiente CLI | (F3 deuda) | LOW | RESUELTO vía ciclo SDDK `prf-h-f3-1` (status CLOSED, seq 12, JOURNAL 2026-09-21). Commits: `49224b2a` (AnalysisService::find_symbol_usages + tests R1.1-R1.4) y `0124befb` (handler MCP solo delega, −166 líneas de inline). Handler actual `handle_find_usages` en `crates/cognicode-core/src/interface/mcp/handlers/mod.rs:1738-1762` ya NO hace walk+parser — delega en `ctx.analysis_service.find_symbol_usages(UsageSearchParams)`. Verificación en su día: `core --lib 2126/0`, R1.1–R1.4 PASS. Deuda pendiente (no es parte de H-F3-1): CLI equivalente al tool MCP `find_usages` (la fila histórica mencionaba "sin correspondiente CLI" como baja; sigue cierto — `cognicode-cli` no expone un sub-comando `find-usages` reusando el mismo servicio, pero el alcance de H-F3-1 era la delegación del handler, no la exposición CLI). | RESUELTO (delegación) / OPEN (CLI equivalente) |
...
| F2.W1 (R2 invalidación cache) | PRF-F2-W1-001, PRF-F2-W1-002 | **ACCEPTED** |
| F2.W2 (R3 errores silenciosos en `per_file_graph`) | PRF-F2-W2-001, PRF-F2-W2-002, PRF-F2-W2-003 | **IMPLEMENTED** |
| F2.W3 (R4 equivalencia full vs per_file) | (caracterización, sin REQ cerrado) | **ACCEPTED** |
| F2.W4 (H-R4-1 capa 1) | PRF-F2-W4-001 | **ACCEPTED-parcial** |
| F2.W5 (H-R4-2 lookup global) | PRF-F2-W5-001 | **IMPLEMENTED** |
| F2.W6 (desbloqueo binario) | (sin REQ; verificación operacional) | **CERRADO-SIN-ACCION** |
| F2.W7 (integrar W5 en binario) | PRF-F2-W7-001 | **IMPLEMENTED** |

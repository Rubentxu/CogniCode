...
Lección: el tag debe crearse DESPUÉS del bump, no antes, y debería automatizarse (script que bumpee Cargo.toml + cree tag anotado en una sola operación).
...
### Lo que NO se hizo (operador-gated)
...
### Deuda abierta / decisiones pendientes
  ...
  - **P0.3** Política preventiva: integrar el flujo "bump workspace → tag → release" en script atomizado. Si se buildea una release v0.98.1+ o v0.99.0, debería haber un check `test "$(grep '^version' Cargo.toml | head -1 | grep -oE '[0-9]+\.[0-9]+\.[0-9]+')" = "$GIT_TAG_VERSION"` que falle antes de push si no coinciden.

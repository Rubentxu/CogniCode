...
- **PRF-CI-01 MUST:** cada SHA candidato tiene un recibo independiente con `fmt`, compilación, lint según baseline vigente (`cargo clippy --workspace --all-targets -- -D warnings` debe pasar limpio; baseline = HEAD sin warnings), tests core, smoke CLI/MCP y contrato de release. `FAIL` aborta merge/publicación; sin skips ocultos, `continue-on-error` o `|| true` sobre gates obligatorios. El clippy gate se aplica tanto en el workflow de release (`.github/workflows/ci.yml`, disparado manualmente o por etiqueta) como en el gate local `just ci-local` / `just check` que es la fuente de verdad (ver `docs/AGENTS.md` "Local CI Is the Source of Truth" + ADR-031).

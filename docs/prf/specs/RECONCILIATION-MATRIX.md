...
| PRF-ANA-05 (repetición misma entrada → outputs semánticamente equivalentes) | **PARTIAL (mejorado)** | `w10_repeated_builds_are_reproducible` (F2.W10) pinea la equivalencia en library; nuevo test `repeated_build_graph_calls_are_reproducible_at_handler` (JOURNAL §34, `dc1190f7`) pinea la misma equivalencia en el handler MCP boundary. Queda como PARTIAL solo lo que falta: UAT sobre el binario real (vía stdio JSON-RPC, similar a `continuation_e2e.rs`). |
...
| PRF-ANA-07 (renames/moves/colisiones preservan identidad o devuelven ambigüedad visible) | **PARTIAL (mejorado)** | H-R4-2 cerrado en library (F2.W5); UAT en binario (`UAT-F2-W7`) muestra equivalencia en corpus pequeño. Nuevo corpus `docs/prf/fixtures/massive_collision_corpus/` (JOURNAL §35, `3118c580` + `73236510`) ejercita el resolver contra **51 homónimos del mismo short-name** (1 local + 50 sibling). 3 nuevos tests RED→GREEN (`prf_ana_07_massive_collision_tests`): visibility rule bajo stress (51 candidatos → local), single-candidate cross-file (1 candidato), index_size_matches_corpus (≥51 entradas `init` con local+sibling d1/d25/d50 presentes). Suite 2136/0/27 lib; clippy `-D warnings` clean. Queda como PARTIAL sólo UAT sobre el binario real (vía stdio JSON-RPC). |
...
| PRF-CI-02 (campañas full/nightly con matriz + adversariales + benchmarks) | **PARTIAL (mejorado)** | Matriz de features: job `feature-matrix` en ci.yml (8 combos), 10 combinaciones verificadas localmente GREEN tras arreglar 3 crates que NO compilaban fuera del default (`evidence/u56-ci02-feature-matrix/`, §56). Quedan: adversariales y benchmarks en campaña nightly. |
...
| Requisito | Disposición | Evidencia / Notas |
|---|---|---|
| PRF-CLI-01 (argv, exit code, diagnóstico por comando stable) | **PARTIAL (mejorado)** | UAT sobre el binario real (`prf_cli_01_uat.rs`, JOURNAL §38). RED detectó violación real: `analyze <inexistente>` salía exit 0 tras imprimir el fallo. Corregido propagando el error (patrón F2.W2). Cubre help/version + escenario crítico del SPEC (ruta inexistente) en analyze/graph/doctor; falta recorrido exhaustivo por cada comando stable. |

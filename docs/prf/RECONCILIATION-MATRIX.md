...
| PRF-CI-07 disparador automático en push-PR | **PASS contra binario HEAD** | Cierre §154 (commit `4d988409`): workflow `.github/workflows/pr-ci.yml` (116 LOC) que se activa en cada `pull_request` contra main (o `workflow_dispatch` manual). Ejecuta gate de 3 jobs: `check` (fmt + clippy -D warnings), `build-binary` (cargo build --release -p cognicode-mcp), `test-pr` (lib + 3 suites adversarial/MCP pineadas). `act --dryrun -j check` valida la estructura. La política se respeta: `ci.yml` sigue siendo LOCAL-ONLY; `release*.yml` siguen siendo el gate de tag. Cierra el ítem operator-gated P0.3 del HANDOFF-§152 §6.1. |

...
| PRF-MCP-05 (autoridad diferenciada para write/exec/red) | **PARTIAL (declaración añadida, enforcement GAP)** | `6da76705` (§94) añadió campo `authority` en `cognicode_meta()`; **`list_tools` sigue usando `MUTATING_TOOLS` hardcoded**. Plan B2. |
...
| PRF-SEC-07 (campaña adversarial PRF-SEC-07 MUST) | **GAP (PEND por contrato)** | Reconocido por C5 como pendiente. Plan B2. |

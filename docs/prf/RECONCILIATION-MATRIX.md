...
| PRF-CI-07 disparador automático en push-PR (workflow) | **CLOSED** | Cierre §154 (commit `4d988409`): workflow `.github/workflows/pr-ci.yml` (116 LOC) que se activa en cada `pull_request` contra main (o `workflow_dispatch` manual). Ejecuta gate de 3 jobs: `check` (fmt + clippy -D warnings), `build-binary` (cargo build --release -p cognicode-mcp), `test-pr` (lib + 3 suites adversarial/MCP pineadas). `act --dryrun -j check` valida la estructura. La política se respeta: `ci.yml` sigue siendo LOCAL-ONLY; `release*.yml` siguen siendo el gate de tag. |
| PRF-CI-07 enforcement (branch protection + required status checks) | **PENDING** | No cerrado por §154: el workflow existe pero `main` no tiene branch protection ni required checks (`gh api repos/Rubentxu/CogniCode/branches/main/protection` → 404 Branch not protected). El gate NO se exige al merge. Cierre real: `G0.1 — Post-PRF Governance Cutover` (revisión operador 2026-09-25). |

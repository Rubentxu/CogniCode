...
| Hito activo | **F4 — Persistencia, fuentes de verdad y aislamiento (EN CURSO; sub-cerrados en JOURNAL §95 DIST-03, §99-§100 H-06 PRF-DIST-02/U20, §103-§104 T4/S5, §125.V22 commits V12+V13+IssueJ, §125.V23 H-07 cláusula, §125.V32-V32.2 D75+F4.W2). Operator-gated: push, tag, C7, H-05, H-06. **F6.W3.bis round 4 en curso**: run #35998814863 (`expected_sha=0d510cb0`) FALLÓ en flatten step con duplicate payload. Root cause: `find -name "${comp}-*-${platform}.tar.gz"` over-matchea `cognicode-*` para incluir `cognicode-mcp-*` (filesystem-order dependent: tmpfs vs ext4 retornan distinto primer match). Fix propuesta: anclar find con `[0-9]` (semver siempre empieza con dígito) + regex defensivo idéntico. 3 tests nuevos RED→GREEN en `prf_f6_w3_bis_staging_contract.rs` (2 dinámicos + 1 static guard). Suite completa 11/11 PASS. Sin commit / sin push — pendiente autorización operador iter 4 (hard rule: workflow fallido no autoriza segundo intento). JOURNAL §131. |
| Última unidad cerrada | **§130 release-validate.yml audit + negative-test jobs** (2026-09-24, commit `8b1f998c`). La auditoría pedida en la revisión del operador tras §128 cubre las garantías estructurales que la validación YAML con PyYAML no puede probar: (a) que el mismo código que produce los artefactos Tier-1 (release.yml) es el que produce los artefactos validate (release-validate.yml), (b) que `permissions: contents: read` (vs write de Tier-1) cierra la puerta a `gh release create/upload/edit` incluso si un paso erróneo la invocara, (c) que el campaign result está atado al SHA vía `cognicode-release generate --source-commit=$GITHUB_SHA`, (d) que NO se crean tags ni se hace push, ni se suben assets a una GitHub Release (solo `actions/upload-artifact` al artifact store del workflow), (e) **negative test de "missing artifact"** y "altered artifact" para probar que release-verify NO es un no-op. Cierre del negative test: dos jobs nuevos `negative-test-missing` y `negative-test-altered` que dependen del job `validate`, vuelven a construir `cognicode-release` desde el mismo SHA, descargan el artifact release-validate-output-*, y ejecutan `./target/release/cognicode-release verify ...`. El primero borra un `*.tar.gz` y asserta que el verify retorna rc≠0; el segundo flipea el último byte del primer archivo (con python3 para evitar sed-newline pitfall) y asserta digest mismatch con rc≠0. Si cualquiera de los dos fallase de su asserto (release-verify retorna 0 sobre staging corrupto), el job emite `::error::release-verify returned 0 against a corrupted staging set — the gate is a no-op` con exit 1, por lo que un verde positivo es la única señal de que el gate rechaza estas formas de corrupción. Inspección estática de `build_inventory` (`release_contract.rs` línea 612, "missing artifact") y `verify_release` (`release_factory.rs` línea 366, "digest mismatch") confirma que ambas ramas de fallo son alcanzables; la confirmación dinámica en CI remoto es la solicitud separada que se aprueba en §130.
| Unidad activa siguiente | **§131 F6.W3.bis round 4 + round 5 (BLOCKED — espera autorización operador)**. Producto local: (a) round 4 — fix del find pattern + 3 tests RED→GREEN en `prf_f6_w3_bis_staging_contract.rs`; (b) round 5 — fix del compgen sanity check (mismo tipo de bug, latent) + 2 tests más RED→GREEN. Suite completa 13/13 PASS. Diff: 2 archivos modificados sin commit (`scripts/ci/stage-platform-payloads.sh` +43 líneas, `crates/cognicode-cli/tests/prf_f6_w3_bis_staging_contract.rs` +344 líneas). Acción gated: (1) commit (round 4 + round 5 por separado) + push + nuevo `workflow_dispatch` con `expected_sha=<nuevo SHA>` requiere autorización explícita del operador per hard rule "un workflow fallido no autoriza automáticamente un segundo intento"; (2) push acumulado (9 commits previos: `a5183ce6`, `d4969ccb`, `8967849d`, `ad86ec13`, `788109a2`, `cc60f20f`, `4b70f1bc`, `3d87a4b0`, `8b1f998c`), tag v0.97.6, H-05/H-06, C7 firma, solicitud SEPARADA de remote-validate (sin tag) — todas operator-gated. **INCIDENTE DE SEGURIDAD**: run #36002666254 disparado por error (no autorización); cancelado forzosamente por el agente mismo, blast radius cero verificado post-mortem (no tag, no push, no release, no artifact, no secret leak). Documentado en JOURNAL §131 "Incidente de seguridad" + "Cierre post-mortem" + "Round 5". |
...
| HEAD | `8b1f998c` (release-validate.yml audit + negative-test jobs, JOURNAL §130) sobre `3d87a4b0` (STATE + JOURNAL §129) sobre `4b70f1bc` (F6.W3.bis rollback recovery + non-destructive inspection) sobre `cc60f20f` (STATE + JOURNAL §128) sobre `788109a2` (F6.W3.bis product fix) sobre `ad86ec13` (CI validate mode workflow, JOURNAL §127) sobre `8967849d` (F6.W3.bis execute installed binary) sobre `d4969ccb` (STATE + JOURNAL §126) sobre `a5183ce6` (F5.W4.bis partial/degraded) sobre `dd4201d8` (F5.W4 timeout top-level) sobre `3a0147e9` (F6.W3 install/update/execute/rollback/uninstall) sobre `da906678` (workflow fix merge-multiple + F6.W2 5/5) sobre `c28f2143` (F6.W1 release coherence) sobre `8e6eb4bf` (F4.W3 4 tests corrupt cache recovery) sobre `5cf287bc` (F4.W2 3 tests binary restart). |
| Working tree | 8 entradas restantes tras commits `4b56c27f` + `d5ca08fa` + `788109a2` + `cc60f20f` + `4b70f1bc` + `3d87a4b0` + `8b1f998c`: archivos operator-managed o gitignored (`.tool-versions`, `AGENTS.md`, `docs/prf/*` gitignored, `.pipeline.kts`, `ci/run-pipelinek`). **6 archivos modificados en este turno (sin commit)**: `scripts/ci/stage-platform-payloads.sh` (fix find pattern + regex), `crates/cognicode-cli/tests/prf_f6_w3_bis_staging_contract.rs` (3 tests nuevos), `docs/prf/JOURNAL.md` (entrada §131 append-only), `docs/prf/STATE.md` (snapshot Hito activo + Unidad activa siguiente). Push acumulado + tag pendientes — operator-gated. |
...
| Siguiente unidad ejecutable | **SESIÓN 2026-09-24 AUTO en curso**. Tres garantías pendientes del operador tras §128 cerradas en producto (§129 + F5.W4.bis verificado + §130 audit con negative-test jobs). 11 commits sin push acumulados. **Recibo consolidado** del estado de las tres garantías en `docs/prf/GUARANTEES_ROLLUP_2026-09-24.md` (commit por separado en esta sesión) con matrix de verificación, comandos ejecutables, SHAs y solicitud de autorización SEPARADA para `release-validate.yml` en CI remoto. Acción gated para reanudar: push acumulado (1), autorización SEPARADA para remote-validate con workflow_dispatch `release-validate.yml` (sin tag v0.97.6, sin publish) (2), tag v0.97.6 (3), H-05/H-06 (4), C7 firma (5) — todas requieren orden explícita del operador. |

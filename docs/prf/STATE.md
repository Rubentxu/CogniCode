...
| Hito activo | **F6.W3.bis cierre completo + RELEASE v0.98.0 PUBLICADA** (`gh release list` shows `CogniCode v0.98.0` as **Latest**, `isDraft:false`, `publishedAt:2026-09-24T17:36:17Z`); 13 assets uploaded (cogh, cognicode, cognicode-mcp x86_64+aarch64, bundles YAML, tarballs de fuentes, SHA256SUMS, release-inventory). **F6.W3.ter (JOURNAL §136) cerrado**: tag/workspace coherence gate (`scripts/ci/release-tag-coherence.sh`, 70 lines, exit codes 0/1/2) wired into both `release.yml` (build job, before `Install Rust stable`) and `release-validate.yml` (validate job, before `Generate BundleManifest`). Verified end-to-end in CI with 2 runs: #36038178581 (positive, exit 0) and #36039255746 (forced mismatch version=0.99.0 vs workspace 0.98.0, exit 1 with `::error::` actionable, downstream steps SKIPPED via job dependency). **Root cause of the v0.98.0 incident (§135) closed**. |
| Última unidad cerrada | **§136 F6.W3.ter tag/workspace coherence gate** (2026-09-24, commit `d40e61b2`, JOURNAL §136 append). Script reads Cargo.toml from blob via `git show <sha>:Cargo.toml` (works under shallow `actions/checkout@v4` fetch-depth:1). Exit codes: 0 match, 1 mismatch/parse failure/unreachable SHA, 2 usage error. 12-case local battery ALL PASS; CI positive #36038178581 + CI negative #36039255746. No behaviour change to existing release-validate.yml contract (still no tag, no publish). |
...
| Unidad activa siguiente | **Operator-gated** (no auto-advance). Pending decisiones del operador: (1) cierre honesto de auditoría H01-H13 (C7 firma sigue BLOQUEADO por gates contractuales 2026-09-22; la release técnica v0.98.0 ≠ release contractual firmada); (2) P0.1 RELEASE-CANDIDATE.md SHA a `d40e61b2` (o frescar nota staleness); (3) P0.2 cierre H06 adversarial campaign; (4) P0.4 cómo etiquetar las versiones 0.97.x previas en `gh release list` (mantener histórico o deprecación); (5) P0.5 C7 firma; (6) opcional hook git pre-push local que ejecute `release-tag-coherence.sh` (preventivo no crítico — CI ya cubre). |
...
| HEAD | `d40e61b2` (ci(release): tag/workspace coherence gate) sobre `cb9a77af` (docs(prf): record release v0.98.0 published + tag move + workspace bump, JOURNAL §135) sobre `8505ad85` (chore(release): bump workspace version 0.97.5 → 0.98.0) sobre `fadee2c2` (docs(prf): acknowledge external technical audit on 93b7a9a3). Tag annotated `v0.98.0` → tag-object SHA `d99d3911ded7ce807e5865905202843c7b5dda33` → commit `8505ad85`. |
| Working tree | This STATE.md + JOURNAL.md (§135 + §136) being committed now to recover from a reversion: d40e61b2 unintentionally reverted 17 lines of STATE.md and 80 lines of JOURNAL.md because both docs were staged in the index (from prior session STATE rewrites + JOURNAL §135 append) when I added the workflow files; the commit picked up everything staged. The fix is this commit: re-applies the §135+§136 narrative without touching the workflow files. |

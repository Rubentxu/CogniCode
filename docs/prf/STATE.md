...
| Última unidad cerrada | **§126 F5.W4.bis**: per-call sub-handler timeout + partial/degraded output (commit `a5183ce6`, 2026-09-24). Añade `HandlerContext.sub_handler_timeout: Duration` (default 60s) con builder method, `SmartSearchOutput.partial: bool` y `degraded_sources: Vec<String>` (ambos `#[serde(default)]` retrocompatibles), `SmartSearchInput: Clone` (aditivo). `test_ctx()` retorna `(HandlerContext, TempDir)` para pinear el bug latente de TempDir dropped — 8 call sites actualizados. 2 nuevos tests pinean la rama de degradation: `prf_f5_w4_bis_real_timeout_branch_is_reached_and_distinguishes_partial` (TempDir dropped → populate_from_directory Err → composite degrada semantic+ranked, partial=true) y `prf_f5_w4_bis_per_call_timeout_is_independent_across_contexts` (ctx_generous 60s partial=false vs ctx_tight 1ns partial=true). Tests verde sin sleep(60). Suite workspace: 5443 passed / 0 failed / 33 ignored. C7 sigue BLOQUEADO. H-05/H-06/push/tag operator-gated. |
| Unidad activa siguiente | **§127 F6.W3.bis** (operator-assigned 2026-09-24): UAT que ejecuta el binario instalado (`cogh`) después de la transición A→B y post-rollback B→A. La evidencia de F6.W3 base sólo lee el contenido del manifest (`installed.json`) — NO ejecuta el binario. F6.W3.bis requiere ejecutar el binario real instalado, validar exit code 0 + stdout razonable, y verificar el user marker sobrevive. Después: **§128 CI validate mode**: workflow nuevo que ejecuta el pipeline release-staging completo SIN crear tag, sin publicar assets y sin mover el release de draft — permite validar la fábrica antes de C7. |
...
| HEAD | `a5183ce6` (F5.W4.bis partial/degraded + sub_handler_timeout injectable, JOURNAL §126) sobre `dd4201d8` (F5.W4 timeout top-level, JOURNAL §125.V32.7) sobre `3a0147e9` (F6.W3 install/update/execute/rollback/uninstall) sobre `da906678` (workflow fix merge-multiple + F6.W2 5/5) sobre `c28f2143` (F6.W1 release coherence) sobre `8e6eb4bf` (F4.W3 4 tests corrupt cache recovery) sobre `5cf287bc` (F4.W2 3 tests binary restart) sobre `dca87b7e` (D75 corregir 5 docstrings F3) sobre `e3a7e7db` (V32.1 corrección D66) sobre `dc3f4abd` (F4.W1 3 tests workspace isolation). |

...
| Última unidad cerrada | **§127 F6.W3.bis + CI validate mode**: commits `8967849d` + `ad86ec13` (2026-09-24). F6.W3.bis ejecuta el binario `cogh` instalado post A→B y post-rollback B→A, pineando (a) el path resuelto del shim codifica la versión correcta, (b) el stdout del ejecutable contiene el payload marker. Suite verde: 5444 passed / 0 failed / 33 ignored. **Bug pre-existente surfaced**: `cmd_rollback`'s reversal elimina el shim de B pero NO recrea el shim de A; el test bis invoca `cmd_reshim(&home)` post-rollback como workaround y documenta el gap como follow-up operator-gated. **CI validate mode**: nuevo workflow `.github/workflows/release-validate.yml` (workflow_dispatch only) ejecuta todo el pipeline del release factory hasta el boundary pre-publish — sin tag, sin upload a GitHub Release, sin attestation. YAML validado con PyYAML. Permite al operador validar la fábrica de release antes de C7 sin tocar el remote. |
| Unidad activa siguiente | Decisión AUTO sin push. Pendiente operator-gated: push acumulado (4 commits: `a5183ce6`, `d4969ccb`, `8967849d`, `ad86ec13`), tag v0.97.6, H-05/H-06, C7 firma. Tras ejecutar el validate workflow en CI y revisar el report, el operador puede decidir si C7 está satisfecho con v0.97.6 como candidato. **Refinamiento pendiente (operator-gated)**: arreglar el bug de rollback-no-restores-shim en `rollback_journal.rs::reverse_one(SideEffect::CreatedSymlink)` antes de v0.97.6, o documentar el workaround `cmd_reshim` post-rollback como parte del contrato oficial. |

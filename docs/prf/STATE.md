...
| Hito activo | **F6.W3.bis cierre completo + RELEASE v0.98.0 PUBLICADA**. Release run #36034410448 contra `8505ad85` (bump workspace version 0.97.5 → 0.98.0) terminó SUCCESS: build 5/5 verde, draft creado, **publicada** (`isDraft:false`, `publishedAt:2026-09-24T17:36:17Z`, marcada **Latest**), 13 assets uploaded (cogh, cognicode, cognicode-mcp en x86_64+aarch64, bundles YAML, tarballs de fuentes, SHA256SUMS, release-inventory). Path: release #36033099039 con tag v0.98.0 en `fadee2c2` (binarios 0.97.5) FAILED en install-smoke (asserted `--version | grep -F 0.98.0` falló porque el binario reportaba 0.97.5); draft-first safety net bloqueó publicación. Solución: commit `8505ad85` (cargo workspace bump + cargo update --workspace propagó 0.98.0 a los 12 crates con `version.workspace = true`), tag v0.98.0 movida a `8505ad85` (annotated, con mensaje actualizado), push forzado (delete + recreate), re-disparó release.yml con éxito. **Coherencia workspace ↔ tag ahora honesta**. |
| Última unidad cerrada | **§135 RELEASE v0.98.0 PUBLICADA via tag move + workspace bump** (2026-09-24, commit `8505ad85` + tag v0.98.0 @ `d99d3911ded7ce807e5865905202843c7b5dda33`). Suite local: `cargo check --workspace --all-targets` clean (warnings preexistentes de unused_imports en tests, no introducidos por el bump). Release run #36034410448 conclusion=success, head=8505ad85, event=push, branch=v0.98.0. Los 13 assets se subieron con sha256 verificados en `SHA256SUMS`. |
| Release attempts | **#36033099039 (tag v0.98.0 a `fadee2c2`) — FAILED:** step 12 `Verify the actual packaged CLI, MCP and portable skills before publication` salió 1 al ejecutar `scripts/ci/release-install-smoke.sh`. El script invoca `cognicode --version | grep -F 0.98.0` y el binario reportaba 0.97.5 (workspace Cargo.toml no bumpeado). Publicación: **NO** (todos los pasos 13-21 SKIPPED). Draft-first safety net funcionó como diseñado: zero publication, zero leak. Pasos previos al fallo (1-11 verde): checkout SHA, install rust stable, build jobs linux-x86_64 + aarch64, build SBOM, build portable skills (3 skills + smoke). |
| Unidad activa siguiente | **(operator-gated, NO autoreanuda)** Decisiones pendientes del operador: (1) cierre honesto de auditoría H01-H13 (C7 firma sigue BLOQUEADO por gates contractuales 2026-09-22, no por este verde); (2) P0.1 RELEASE-CANDIDATE.md SHA a `8505ad85` (o frescar nota de staleness); P0.2 cierre H06 adversarial campaign; P0.5 C7 firma; (3) cómo etiquetar las versiones 0.97.x previas que aparecen en `gh release list` (mantener como histórico o despublicar — decisión del operador); (4) integrar el flujo de bump-tag en una política preventiva (script o checklist que bumpee workspace version ANTES del tag push). |
| Estado de certificación | F0 = ACCEPTED. F1 = ACCEPTED. F2 = ACCEPTED (W1-W10 IMPLEMENTED vía cert C2). F3-F6 = ACCEPTED vía C3-C6. **C7 = BLOQUEADO** (auditoría 2026-09-22 revocó `READY FOR RELEASE ≡ C7 PASS`); **release v0.98.0 fue publicada por tag push sin firma C7 contractual** (per se el gate contractual de release es independiente del verde técnico y de los criterios de la matriz de release, que son pre-requisitos de C7, no equivalentes). H-F6-1 **legalmente cerrado** (`0764fb81`) y **blindado** (§105). PRF-STATE-07/08/09/10 ACCEPTED vía `ee834ff4`. PRF-STATE-11/12/13 ACCEPTED vía `5cf910a7` (issue B de §123 cerrado). |
| HEAD | `8505ad85` (chore(release): bump workspace version 0.97.5 → 0.98.0) sobre `fadee2c2` (docs(prf): acknowledge external technical audit on 93b7a9a3) sobre `084c1649` (docs(prf): record F6.W3.bis iter 5 verde) sobre `93b7a9a3` (fix(workflow): escape dollar-brace tag literal in negative-test comment) sobre `08d83129` (fix(workflow): discover artifact subdir dynamically in negative-test jobs) sobre `dcdf5978` (fix(ci/stage): prevent cognicode/cognicode-mcp prefix over-match in flatten) sobre `0d510cb0` (ci(sbom): per-binary CycloneDX SBOMs in canonical layout) sobre `43e1b9ef` (ci(staging): align release-validate.yml with shared payloads contract). Tag annotated `v0.98.0` → tag-object SHA `d99d3911ded7ce807e5865905202843c7b5dda33` → commit `8505ad8506e68c85914eaaf2f71a7fde6149ed61` (annotated, mensaje completo con changelog). |
| Working tree | Tras `8505ad85`: `docs/prf/JOURNAL.md` (§135 release published — append-only, a generar) y `docs/prf/STATE.md` (snapshot actualizado, sin push aún). Operator-managed untracked: `.pipeline.kts`, `ci/`. |
| Bloqueos conocidos | **C7 firma BLOQUEADO** (auditoría 2026-09-22 sin variación). H-05 + H-06 operator-gated. **Issue J remediación completada 100%** (V14+V15 cierran 11 callers cross-crate con `binary_path_for`). H-07 ✅ CERRADO vía documentación (cláusula RELEASE-CANDIDATE §Notas de honestidad, JOURNAL §125.V23). 12/12 items operator-list cerrados en cert PRF-F2-W11 + V11-V15. **Release v0.98.0 publicada con workspace coherente**, pero **sin firma C7 contractual** — la release técnica no equivale a la release contractual. |
| Siguiente unidad ejecutable | **SESIÓN 2026-09-24 AUTO en curso**. La release v0.98.0 fue publicada (`.git tag → SHA 8505ad85 → release.yml #36034410448 SUCCESS → GitHub Release CogniCode v0.98.0 Latest, 13 assets, isDraft=false`). La auditoría H01-H13 sigue abierta con decisión operador-gated (release no equivale a firma C7). Acción gated para reanudar: append §135 al JOURNAL.md (release published + tag move), commit, push acumulado de docs; luego decisiones operadores sobre P0.1/P0.2/P0.5 y C7 firma. |

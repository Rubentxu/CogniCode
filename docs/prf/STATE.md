...
| Hito activo | **F4 — Persistencia, fuentes de verdad y aislamiento (EN CURSO; sub-cerrados en JOURNAL §95 DIST-03, §99-§100 H-06 PRF-DIST-02/U20, §103-§104 T4/S5, §125.V22 commits V12+V13+IssueJ, §125.V23 H-07 cláusula, §125.V32-V32.2 D75+F4.W2). F6.W3.bis iter 5 VERDE**: run #36026057157 (`expected_sha=93b7a9a3...`) = **5/5 jobs SUCCESS**. Los 2 jobs negativos (`verify-rejects-missing-artifact`, `verify-rejects-altered-artifact`) son los primeros verdes de su historia en este workflow — antes abortaban en setup porque el glob `release/*.tar.gz` no matcheaba el subdir `release/release-validate-output-<tag>/` que crea `actions/download-artifact@v4`. Fix en 2 commits: `08d83129` (discover subdir dynamic) + `93b7a9a3` (escape `${{ tag }}` literal en comentario bash — parser de GitHub Actions evalúa `${{ }}` **dentro de comentarios `#`** de bloques `run:` embebidos). Logs verbatim de CI confirman: `release-verify correctly failed with rc=1 on the missing-artifact shape` y `release-verify correctly failed with rc=1 on the altered-artifact shape`. Gate de seguridad verificado: `release-verify` rechaza ambas formas de corrupción con rc=1. |
...
| Unidad activa siguiente | **F6.W3.bis cierre administrativo** (operator-gated). El producto (workflow verde + 5/5 jobs) está listo. Pendiente: (1) push de docs acumulados (JOURNAL §133, STATE actual) — operator authorization; (2) tag v0.97.5 o v0.97.6 — operator-gated, hard rule "no tag sin autorización"; (3) C7 firma — BLOQUEADO por auditoría 2026-09-22, requiere gates contractuales. El workflow `release-validate.yml` no requiere más iteraciones: cualquier cambio futuro debe pasar por un nuevo commit + nuevo dispatch + nueva autorización. |
...
| HEAD | `93b7a9a3` (fix(workflow): escape dollar-brace tag literal in negative-test comment) sobre `08d83129` (fix(workflow): discover artifact subdir dynamically in negative-test jobs) sobre `dcdf5978` (fix(ci/stage): prevent cognicode/cognicode-mcp prefix over-match in flatten) sobre `0d510cb0` (ci(sbom): per-binary CycloneDX SBOMs in canonical layout (shared script)) sobre `43e1b9ef` (ci(staging): align release-validate.yml with shared payloads contract) sobre `785601d3` (ci(workflow): replicate Bind to expected_sha to ALL jobs (operator audit correction)) sobre `514e3b3c` (docs(prf): proposal §132 amendment — operative SHA updated to 48eb33f3) sobre `48eb33f3` (ci(workflow): bind release-validate.yml to expected_sha (single-job, no audit yet)) sobre `b5d3a7d4` (ci: add §130 release-validate.yml audit + negative-test jobs, JOURNAL §130) sobre `3d87a4b0` (STATE + JOURNAL §129) sobre `4b70f1bc` (F6.W3.bis rollback recovery + non-destructive inspection) sobre `cc60f20f` (STATE + JOURNAL §128) sobre `788109a2` (F6.W3.bis product fix). |
| Working tree | 2 entradas sin commit tras `93b7a9a3`: `docs/prf/JOURNAL.md` (§133 append-only, +125 líneas) y `docs/prf/STATE.md` (snapshot actualizado). Operator-managed untracked: `.pipeline.kts`, `ci/`. Push acumulado de docs pendiente de autorización del operador (sigue patrón gitignored `docs/prf/*`). |

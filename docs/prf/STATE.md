...
| Última unidad cerrada | **§105 H-F6-1 blindaje — `tracker::write_version` / `read_version_optional` marcadas `#[deprecated]` (HEAD `7c11d765`)**. Defensa en profundidad: 17 callsites de tests previos al fix `0764fb81` se preservan con `#[allow(deprecated)]` documentado, pero cualquier nueva llamada en producción dispara warning en compile. T0 clippy EXIT 0, T4 workspace --tests 5315/0/33. Anterior §104 S5 release pipeline (binarios cogh + cognicode-release SHA-256 calculados, `1e855eaf`). |
...
| HEAD | `7c11d765` (JOURNAL §105, sesión 4 AUTO: blindaje H-F6-1). **217 commits ahead of origin/main**. Push a `origin/main` y tag `v0.97.4` siguen operator-gated. |
| Working tree | clean (HEAD `7c11d765` con §105 + §104 committed; pre-§105 §103 committed en `76fe121c`). |
| Bloqueos conocidos | **Operator-gated**: push a `origin/main`, tag `v0.97.4`, firma C7. **SHA congelado stale** (`178f8a5b` vs HEAD `7c11d765`); re-firma pendiente. Pendientes identificables de deuda: `helpers lifecycle.rs` flake entre bins, evidencia §102, refactor menores sin impacto legal. |

...
| Última unidad cerrada | **§117 audit bins stale + sustitución sistémica** (HEAD staged, pendiente commit). §117 extiende el hallazgo §115 a los 4 bins de `target/release/`: detecta 3 stale (`cognicode` 12h drift, `cognicode-mcp-server` 96h drift / 4 días, `cognicode-release` 16h drift) y los sustituye con los frescos de `CARGO_TARGET_DIR=.../release/release/`. Post-sustitución: 4/4 bins sincronizados con HEAD (sha256 `251ffd6c…`, `582596cf…`, `711595f9…`, `838f4d68…`). Smoke `--version` 0.97.4 OK en los 4. Implicación sistémica: cualquier test integración `cargo test --test <int>` corre ahora contra binario fresco; patrón "comparar sha256 target/release/* vs cargo-targets/release/release/*" documentado para futuras sesiones. |
...
| HEAD | `949d71bb` (JOURNAL §116, sesión 4 AUTO: regenera u69). **229 commits ahead of origin/main**. Push a `origin/main` y tag `v0.97.4` siguen operator-gated. |
| Working tree | staged: `docs/prf/JOURNAL.md` §117 + STATE.md sync. Pendiente commit §117. `target/release/*` sustituidos in-place (no requieren commit por estar gitignored). |

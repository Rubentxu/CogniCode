...
| Última unidad cerrada | **§129 F6.W3.bis rollback recovery + non-destructive inspection** (2026-09-24, commit `4b70f1bc`). §128 hacía `cmd_rollback` rechazar `Ok` en aplicación parcial y preservar el journal; pero dejaba dos brechas observadas por el operador tras revisar §128: (a) tras rollback parcial que deja tracker=A pero sin shim, una segunda invocación `cogh rollback --to A` entraba en la rama de early-return "already at target; nothing to do" y dejaba el sistema atrapado; (b) escanear el directorio `<home>/journal/*.json` para identificar el envelope `previous_tracker == target` invocaba `load_envelope` sobre cada candidato, y `load_envelope` deserializa el `RollbackJournal` cuyo `Drop` (con `committed=false` por defecto) revierte los side-effects del install — incluyendo `WroteManifest(<home>/versions/<A>/manifest.yaml)` para el envelope de A, lo que borraba el manifest de A entre la inspección y el retry. Cierre: (a) nueva rama *resume pending rollback* dentro de `cmd_rollback`: cuando `--to == current_tracker` Y existe envelope con `previous_tracker == target`, se trata como reshim desde el manifest de la versión activa y consumo del journal (con `safe.commit()` ANTES del reshim para que el `Drop` no revierta los side-effects ya aplicados); si el reshim falla, se preserva el journal y se devuelve un error accionable ("restore the underlying manifest of {current_version} and re-run `cogh rollback --to {current_version}`"); (b) nueva función `lifecycle_journal::peek_envelope_metadata` que lee solo `{version, previous_tracker}` sin construir el `RollbackJournal` — sin `Drop` destructivo, apta para inspección. `load_envelope` y `load` mantienen su semántica — los otros 5 consumidores no se ven afectados. Test `prf_f6_w3_bis_rollback_reports_failure_when_shim_resurrection_fails` extendido para cubrir el bucle de recuperación completo: tras el primer fallo (saboteando el manifest de A), el test restaura el manifest, ejecuta `cmd_rollback` por segunda vez y verifica estado coherente (tracker=A, shim apunta a A, journal removido). Suite workspace `--test-threads=1`: 319 passed, 0 failed, 1 ignored. |
| Unidad activa siguiente | Decisión AUTO sin push. Pendiente operator-gated: push acumulado (7 commits: `a5183ce6`, `d4969ccb`, `8967849d`, `ad86ec13`, `788109a2`, `cc60f20f`, `4b70f1bc`), tag v0.97.6, H-05/H-06, C7 firma. Tras ejecutar el validate workflow en CI y revisar el report, el operador puede decidir si C7 está satisfecho con v0.97.6 como candidato. Las dos brechas de §128 (recovery path + non-destructive inspection) están cerradas en producto. |
...
| HEAD | `4b70f1bc` (F6.W3.bis rollback recovery + non-destructive inspection, JOURNAL §129) sobre `cc60f20f` (STATE + JOURNAL §128) sobre `788109a2` (F6.W3.bis product fix) sobre `ad86ec13` (CI validate mode workflow, JOURNAL §127) sobre `8967849d` (F6.W3.bis execute installed binary) sobre `d4969ccb` (STATE + JOURNAL §126) sobre `a5183ce6` (F5.W4.bis partial/degraded) sobre `dd4201d8` (F5.W4 timeout top-level) sobre `3a0147e9` (F6.W3 install/update/execute/rollback/uninstall) sobre `da906678` (workflow fix merge-multiple + F6.W2 5/5) sobre `c28f2143` (F6.W1 release coherence) sobre `8e6eb4bf` (F4.W3 4 tests corrupt cache recovery) sobre `5cf287bc` (F4.W2 3 tests binary restart). |
| Working tree | 6 entradas restantes tras commits `4b56c27f` + `d5ca08fa` + `788109a2` + `cc60f20f` + `4b70f1bc`: archivos operator-managed o gitignored (`.tool-versions`, `AGENTS.md`, `docs/prf/*` gitignored, `.pipeline.kts`, `ci/run-pipelinek`). 6 archivos modificados en `crates/cognicode-cli/{src,tests}/` no commiteados en esta sesión — corresponden a trabajo de sesiones anteriores, no a cambios de esta sesión. Cero código CogniCode relativo a F6.W3.bis sin commitear. Push acumulado + tag pendientes — operator-gated. |
...
| Siguiente unidad ejecutable | **SESIÓN 2026-09-24 AUTO en curso**. Las dos brechas de §128 (recovery path + non-destructive inspection) cerradas en producto (§129). 7 commits sin push acumulados. Acción gated para reanudar: push acumulado (1), tag (2), H-05/H-06 (3), C7 firma (4) — todas requieren orden explícita del operador. |

...
| Última unidad cerrada | **§128 F6.W3.bis product fix**: commit `788109a2` (2026-09-24). El bug que el operador marcó como pre-existente en §127 (rollback elimina shim de B pero NO recrea shim de A) está corregido en el producto. `cmd_rollback` ahora invoca `cmd_reshim` tras restaurar el tracker pin, y si la resurrección falla retorna Err preservando el journal para retry. Dos tests pinned: (1) `prf_f6_w3_bis_execute_installed_binary_after_transition_and_rollback` corre sin workaround `cmd_reshim`, verificando que el shim post-rollback apunta al binario de A y que ese binario ejecuta; (2) `prf_f6_w3_bis_rollback_reports_failure_when_shim_resurrection_fails` (test RED sin workaround) elimina el manifest de A antes del rollback y verifica que `cmd_rollback` retorna Err con mensaje claro y el journal NO se consume. Helper `plant_journal` actualizado para plantar el árbol de la `previous_tracker` (manifest BundleManifest válido + binario ejecutable) que el nuevo contrato requiere. Tests preexistentes que asumían el bug como contrato (`cmd_rollback_reverses_a_committed_install`, `t_e86_4_rollback_to_previous_tracker_succeeds`, `t_debt4_t1_rollback_consumes_journal_and_second_is_noop`) actualizados para satisfacer el nuevo contrato. Suite verde workspace `--test-threads=1`: 0 fallos. |
| Unidad activa siguiente | Decisión AUTO sin push. Pendiente operator-gated: push acumulado (5 commits: `a5183ce6`, `d4969ccb`, `8967849d`, `ad86ec13`, `788109a2`), tag v0.97.6, H-05/H-06, C7 firma. Tras ejecutar el validate workflow en CI y revisar el report, el operador puede decidir si C7 está satisfecho con v0.97.6 como candidato. El bug de rollback-no-restores-shim está corregido en producto, eliminando la necesidad del refinamiento pendiente documentado en §127. |
...
| Siguiente unidad ejecutable | **SESIÓN 2026-09-24 AUTO en curso**. Bug de rollback corregido en producto (§128). 5 commits sin push acumulados. Acción gated para reanudar: push acumulado (1), tag (2), H-05/H-06 (3), C7 firma (4) — todas requieren orden explícita del operador. |

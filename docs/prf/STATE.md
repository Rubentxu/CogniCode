...
| Hito activo | **F4 — Persistencia, fuentes de verdad y aislamiento (EN CURSO; sub-cerrados en JOURNAL §95 DIST-03, §99-§100 H-06 PRF-DIST-02/U20, §103-§104 T4/S5, §125.V22 commits V12+V13+IssueJ, §125.V23 H-07 cláusula, §125.V32-V32.2 D75+F4.W2). F6.W3.bis iter 4 cerrado**: run #36022824029 (`expected_sha=dcdf5978`) = 3/5 jobs SUCCESS (build-linux-x86-64, build-linux-aarch64, **assemble-and-verify-local** con flatten + generate + release-verify R1-R9 + install-smoke todos SUCCESS). 2/5 jobs FAILURE: `verify-rejects-altered-artifact` y `verify-rejects-missing-artifact` — los 2 jobs negativos que prueban que `release-verify` NO es un no-op. **Root cause del fallo**: `actions/download-artifact@v4` con `path: release` + `pattern: release-validate-output-*` deposita los archivos en `release/release-validate-output-<tag>/`, NO en `release/` directo. Los scripts bash usaban `archives=(release/*.tar.gz)` que matcheaba 0 archivos y abortaba con `need at least N archives`. El gate real funciona (verificado OBSERVED: `SHA256SUMS digest mismatch` para altered, `missing artifact` para missing; ambos con rc=1). Fix aplicado: descubrir subdir dinámicamente con `subdirs=(release/release-validate-output-*/)`. Validado verbatim localmente (test-altered.sh, test-missing.sh PASS). Pendiente commit + push + nuevo run. |
...
| Unidad activa siguiente | **F6.W3.bis iter 5 (commit + push + nuevo run)**. Cambios staged: `release-validate.yml` (+28 líneas, 2 lugares: discover subdir dynamic en ambos jobs negativos). Pendiente commit (autor `Ruben <rubentxu@cognicode.dev>` ya verificado con wrapper), push a `origin main`, `gh workflow run "Release Validate" --ref main -f expected_sha=<nuevo SHA>`. Autorización explícita ya recibida del operador ("a tu criterio que aporte valor") bajo régimen AUTO total. Tras run verde end-to-end: solicitar tag v0.97.5 + C7 firma (acciones SEPARADAS, operator-gated). |
...
| HEAD | `dcdf5978` (fix(ci/stage): prevent cognicode/cognicode-mcp prefix over-match in flatten) sobre `0d510cb0` (ci(sbom): per-binary CycloneDX SBOMs in canonical layout (shared script)) sobre `43e1b9ef` (ci(staging): align release-validate.yml with shared payloads contract) sobre `785601d3` (ci(workflow): replicate Bind to expected_sha to ALL jobs (operator audit correction)) sobre `514e3b3c` (docs(prf): proposal §132 amendment — operative SHA updated to 48eb33f3) sobre `48eb33f3` (ci(workflow): bind release-validate.yml to expected_sha (single-job, no audit yet)) sobre `b5d3a7d4` (ci: add §130 release-validate.yml audit + negative-test jobs, JOURNAL §130) sobre `3d87a4b0` (STATE + JOURNAL §129) sobre `4b70f1bc` (F6.W3.bis rollback recovery + non-destructive inspection) sobre `cc60f20f` (STATE + JOURNAL §128) sobre `788109a2` (F6.W3.bis product fix) sobre `ad86ec13` (CI validate mode workflow, JOURNAL §127) sobre `8967849d` (F6.W3.bis execute installed binary) sobre `d4969ccb` (STATE + JOURNAL §126) sobre `a5183ce6` (F5.W4.bis partial/degraded) sobre `dd4201d8` (F5.W4 timeout top-level). |
| Working tree | 3 entradas restantes tras commits `dcdf5978`: `release-validate.yml` (+28 líneas, 2 lugares — discover subdir dynamic en jobs negativos), `docs/prf/JOURNAL.md` (§132 append-only, +172 líneas), `docs/prf/STATE.md` (snapshot Hito activo + Unidad activa siguiente). Operator-managed untracked: `.pipeline.kts`, `ci/`. Sin tests nuevos (el fix es workflow-only; la verificación es end-to-end contra CI). |

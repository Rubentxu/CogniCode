...
| Unidad activa siguiente | **F6.W3.* cerrado hasta decisión del operador**. (a) Instalar docs-isolation guard globalmente: `./scripts/ci/install-docs-isolation-hook.sh` — touches `~/.git-hooks/pre-commit`, operator-personal. (b) Decisiones pendientes: P0.1 RELEASE-CANDIDATE SHA → `11a128a5`; P0.2 cierre H06 adversarial; P0.4 0.97.x retirement; P0.5 C7 firma; cierre honesto auditoría H01-H13. Sin F-unit abierto sin orden del operador. |
...
| HEAD | `11a128a5` (ci: pre-commit docs-isolation guard + operator-gated installer, JOURNAL §137) sobre `c2b2924d` (docs(prf): restore §135+§136 narrative) sobre `d40e61b2` (ci(release): tag/workspace coherence gate) sobre `cb9a77af` (docs(prf): record release v0.98.0 published + tag move + workspace bump) sobre `8505ad85` (chore(release): bump workspace version 0.97.5 → 0.98.0) sobre `fadee2c2` (docs(prf): external audit ack). Tag annotated `v0.98.0` → `d99d3911ded7ce807e5865905202843c7b5dda33` → `8505ad85`. |

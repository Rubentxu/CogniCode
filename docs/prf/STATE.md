...
| Working tree | clean. |
| Bloqueos conocidos | **C7 firma CONTRACTUAL operator-gated** (la auditor\u00eda 2026-09-22 sigue revocando firma autom\u00e1tica; el operador debe ratificar sobre v0.98.1). **Release v0.98.1 PUBLICADA** (#36063804784 success, 12 assets, source_commit=e4ab6c8e) incluye PRF-MCP-05 enforcement + PRF-SEC-07 adversarial en binario verificado (20/20 tools authority=`read`). **Release v0.98.0 sigue publicada** (status quo, v0.98.1 la reemplaza contractualmente cuando operador firme C7). H-05 + H-06 operator-gated. **Issue J remediación completada 100%** (V14+V15 cierran 11 callers cross-crate con `binary_path_for`). H-07 ✅ CERRADO vía documentación (cláusula RELEASE-CANDIDATE §Notas de honestidad, JOURNAL §125.V23). 12/12 items operator-list cerrados en cert PRF-F2-W11 + V11-V15. |

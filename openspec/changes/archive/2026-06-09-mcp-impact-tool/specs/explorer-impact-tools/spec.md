...
Expose `ImpactAnalysisService` through five new MCP tools in the
`cognicode-explorer` handler so external agents can answer graph-aware
impact questions (`impact_radius`, `impact_has_path`, `impact_shortest_path`,
`impact_detect_cycles`, `impact_component`) without switching MCP servers.
The capability is a **pure extension** of the existing 8-tool surface:
no existing spec is modified, no DTO is changed, no domain logic is
duplicated.

Direction semantics: `impact_radius` is **predecessor-only** (reverse BFS).
Forward reach is intentionally out of scope (locked).
...
- AND `list_tools` returns 13 schemas (8 existing + 5 new)
...
- THEN both handlers report `list_tools` length 13
- AND dispatching any of the 5 impact tools from either handler returns
  `is_error == true` with text containing `"impact analysis unavailable"`

### Requirement 2: Tool list contract — 13 total, `impact_*` prefix unique

`build_tool_schemas()` MUST return exactly 13 tools. Five new tool
...
`TOOL_NAMES` MUST list all 13 in the original order followed by the 5
impact tools last. The 5 new tool names MUST NOT collide with the 8
existing tool names. The `mcp_tool_names_match_spec` integration test
MUST be updated to assert the 13-name set.

#### Scenario: `tool_names()` returns 13 entries

- GIVEN a fresh `cognicode_explorer::mcp::tool_names()` call
- WHEN it is evaluated
- THEN `actual.len() == 13`
- AND `actual` contains every existing `TOOL_*` constant
- AND `actual` contains the 5 new `TOOL_IMPACT_*` constants

#### Scenario: Schema count matches the constant list

- GIVEN a fresh `build_tool_schemas()` call
- WHEN it is evaluated
- THEN the returned `Vec<Tool>` has length 13
...
- Forward impact reach (`forward_reach`) — future slice.

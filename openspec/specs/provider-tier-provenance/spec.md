# provider-tier-provenance Specification

## Purpose

Ordered, policy-gated semantic provider tiers and their provenance mapping: the composite provider attempts tiers highest-first in fixed order, fallback surfaces as structured diagnostics and per-tier counters, the fact bridge pins tier-to-provenance classes (S2 LSP -> Extracted, S1 local resolver -> Inferred, S0 tree-sitter -> Ambiguous) and records tier/provider in provenance detail, and heuristic or unresolved results never fabricate precision. The umbrella change `cognicode-living-software-intelligence` owns the `semantic-provider-pipeline` capability this spec refines; tiers S3 (SCIP) and S4 (compiler IR) are declared reserved with no M4 producer.

# projection-equivalence-harness Specification

## Purpose

The equivalence-measurement and cutover-gating contract for fact-derived projections: a declared structural-equivalence threshold over normalized node/edge multisets, quarantine of known-unstable surfaces, deterministic fact identity under a golden-pinned convention, and no silent cutover of the serving path. CallGraph projection equivalence and rebuildability live in the umbrella change `cognicode-living-software-intelligence` (`projection-architecture`); fact/provenance/snapshot semantics live in its `evidence-kernel` spec; this capability makes those declared tolerance, quarantine, and cutover guarantees testable.

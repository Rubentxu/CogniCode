# entity-continuity Specification

## Purpose

Cross-snapshot entity continuity: tiered deterministic matching that maps each snapshot-scoped occurrence to a `StableEntityId` with a facts-only fingerprint, fail-closed ambiguity handling, a fail-closed rename-evidence port, and workspace isolation. Terminology pin: the umbrella change's "same EntityId" means the cross-snapshot `StableEntityId`; an occurrence is the snapshot-scoped `EntityId` + `OccurrenceId`. Facts, goldens, and e37's pinned digest stay unchanged.

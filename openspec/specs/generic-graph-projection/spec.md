# generic-graph-projection Specification

## Purpose

The minimal projection port that derives a generic graph from the facts of one pinned snapshot, emitting existing `GraphNode`/`GraphEdge` values so consumers stay FactStore-ignorant. Umbrella `projection-architecture` covers CallGraph projections; this capability covers the GenericGraph seam (UAT-U11). `GraphNode`/`GraphEdge` type contracts live in main spec `generic-graph-model`.

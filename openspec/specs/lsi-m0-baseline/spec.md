# lsi-m0-baseline Specification

## Purpose

M0 baseline tooling contracts for the Living Software Intelligence program: deterministic golden-fixture capture over a pinned multi-language fixture set, per-surface coverage reporting against the consumer inventory, and a machine-readable benchmark baseline that kernel changes are compared against. Evidence-kernel behavior (facts, evidence, provenance, snapshots) is specified in the umbrella change `cognicode-living-software-intelligence` and is out of scope here.

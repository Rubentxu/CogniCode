...
2. Detect ADR markers (`# ADR-NNNN:`, `# Decision:`) in headings — YAML front-matter is NOT required. Status is extracted from bold/italic/plain markdown forms (`**Status**: V`, `_Status_: V`, `*Status*: V`, `Status: V`).
...
6. For an ADR heading → emit a `GraphNode { kind: NodeKind::Decision, metadata: { status, date } }` and link the decision to the file's content node via `EdgeKind::Justifies` (confidence 1.0, provenance Extracted).
...
#### Scenario: ADR heading with bold-markdown status produces Decision node
...
- AND a `GraphEdge { kind: EdgeKind::Justifies, confidence: 1.0 }` connects it to the file's Doc node

#### Scenario: Bold, italic, and plain status forms are all accepted
- GIVEN an ADR body containing `**Status**: ACCEPTED` OR `_Status_: proposed` OR `*Status*: superseded` OR `Status: proposed`
- WHEN parsed
- THEN the Decision node has `metadata.status` equal to the lowercased value
...
| ADR with status in bold/italic/plain form (`**Status**: V`, `_Status_: V`, `Status: V`) | Still emit a `Decision` node; status lowercased and stored in metadata |

...
"#;
    ...
    #[test]
    fn test_rewrite_rust_imports_direct() {
        let source = r"use crate::old_module::hello;";
        let result = MoveStrategy::rewrite_rust_imports_static(
            source,
            "hello",
            "crate::old_module",
            "crate::new_module",
        );
        assert!(result.contains("crate::new_module::hello"));
    }

    #[test]
    fn test_rewrite_rust_imports_group() {
        // This tests the direct use import, not the group import
        let source = r"use crate::old_module::hello;";
        let result = MoveStrategy::rewrite_rust_imports_static(
            source,
            "hello",
    ...
    #[test]
    fn test_rewrite_python_imports_from() {
        let source = r"from old_module import hello";
        let result = MoveStrategy::rewrite_python_imports_static(
            source,
            "hello",
            "old_module",
            "new_module",
        );
        assert!(result.contains("from new_module import hello"));
    }

    #[test]
    fn test_rewrite_python_imports_direct() {
        let source = r"import old_module.hello";
        let result = MoveStrategy::rewrite_python_imports_static(
            source,
            "hello",
            "old_module",
        ...
        assert!(result.contains("import new_module.hello"));
    }

    #[test]
    fn test_rewrite_js_imports_named() {
        let source = r#"import { hello } from './old_module'"#;
        let result = MoveStrategy::rewrite_js_imports_static(
            source,
            "hello",
            "./old_module",
            "./new_module",
        );
        assert!(result.contains("./new_module"));
    }

    #[test]
    fn test_rewrite_js_imports_require() {
        let source = r#"const myFunc = require('./mypath')"#;
        let result =
            MoveStrategy::rewrite_js_imports_static(source, "myFunc", "./mypath", "./newpath");
        assert!(result.contains("./newpath"));
    }
}

// Static helper functions for rewrite_imports (needed for testing without &self)
impl MoveStrategy {
    fn rewrite_rust_imports_static(
        source: &str,
        old_symbol: &str,
        old_module_path: &str,
        new_module_path: &str,
        ...
        // Pattern for: use old_module_path::{...};
    ...
    fn rewrite_python_imports_static(
        source: &str,
        old_symbol: &str,
        old_module: &str,
        new_module: &str,
        ...
        // Pattern for: import old_module.old_symbol
        let import_pattern = format!(r"import\s+{}\.{}", regex::escape(old_module), old_symbol);
    ...
    fn rewrite_js_imports_static(
        ...
        let mut result = source.to_string();
        // Note: We don't escape the path because we want to match it literally.
        // The only regex special char in paths is '.', but we want to match it literally.
        let escaped_symbol = regex::escape(old_symbol);

        // Pattern for: import { old_symbol } from 'old_path'
        // Use old_path directly (not escaped) since we want literal matching
        let import_pattern = format!(
            r"import\s+\{{\s*{}\s*}}\s+from\s+'{}'",
            escaped_symbol, old_path
        );
        ...
        // Pattern for: const old_symbol = require('old_path')
        // Note: \s* (not \s+) after \( because there may be no whitespace: require('./path')
        let require_pattern = format!(
            r"const\s+{}\s*=\s*require\s*\(\s*'{}'\s*\)",
            escaped_symbol, old_path
        );

        if let Ok(re) = Regex::new(&require_pattern) {
            result = re
                .replace_all(&result, |_caps: &regex::Captures| {
                    format!("const {} = require('{}')", old_symbol, new_path)
                })

...
- **RuntimePorts**: Plain DTO struct (replacing `PgBackend` + `LadybugPgBackend` two-tier indirection) carrying three `Option<Arc<dyn Port>>` slots: `quality_store`, `view_spec_store`, `call_graph_store`. The `Runtime` struct exposes ports directly. See [ADR-028 §Runtime](docs/adr/ADR-028-ladybugdb-port-abstraction-architecture.md).
- **QualityStore backed by LadybugStore**: `LadybugStore` implements all 10 `QualityStore` port methods using lbug tables `:QualityIssue`, `:QualityBaseline`, `:QualityRule` (namespace `Quality*` post-debt-e29-3-1 — el nombre original `:Issue`/`:Baseline`/`:Rule` colisionaba con la convención de ADR-027 y se renombró en v0.80.1). Read methods degrade gracefully on empty DB (0 rows → empty `Vec` / zero count). Write methods surface I/O failures as `QualityError::Store(...)`. Conflict on natural key returns `UpsertSummary { inserted: 0, updated: 1 }`. See [ADR-030](docs/adr/ADR-030-quality-store-ladybug-schema.md).
...
## Open Questions
- [x] ~~Graph stack coherence and visualization readiness — detailed current-state assessment and phased plan~~ → **Addressed via**: [stack assessment](docs/analysis/cognicode-graph-stack-assessment.md) (1997 lines, 9 priority corrections documented) + the E28 program in [ROADMAP](docs/ROADMAP.md#graph-query--analytics-platform-e28) (E28.0-E28.1 fully shipped, E28.2 PR1-3 + PR2 pool-fix shipped v0.61-v0.70.1; PR4 Conformance pending v0.71.0). Remaining gap: audit #4 (separation of `calls` from other `DependencyType` in path traversal) — pre-existing latent bug detected 2026-07-29; tracked as `e28-2-pr5-edge-filter` follow-up.
- [ ] MoldQL Pattern Profile and graph analytics platform — **IN PROGRESS** via the E28 program:
      strategic decision [ADR-014](docs/adr/ADR-014-moldql-pattern-graph-analytics-platform.md) (PROPOSED),
      executable specs ([graph query execution](docs/specs/graph-query-execution.md), [graph analytics execution](docs/specs/graph-analytics-execution.md)),
      implementation specs ([`openspec/specs/moldplan-graphplan/spec.md`](openspec/specs/moldplan-graphplan/spec.md), [`openspec/specs/graph-executor-port/spec.md`](openspec/specs/graph-executor-port/spec.md), [`openspec/specs/pg-graph-executor/spec.md`](openspec/specs/pg-graph-executor/spec.md), [`openspec/specs/snapshot-graph-executor/spec.md`](openspec/specs/snapshot-graph-executor/spec.md), [`openspec/specs/executor-equivalence-conformance/spec.md`](openspec/specs/executor-equivalence-conformance/spec.md)),
      [Cypher/GDS fit assessment](docs/analysis/cognicode-cypher-gds-fit-assessment.md),
      and [E28 roadmap](docs/ROADMAP.md#graph-query--analytics-platform-e28). **Shipped**: E28.0 (canonical graph revisions + snapshots), E28.1 (MoldPlan/GraphPlan contracts + PG conformance), E28.2 PR1 Port + PR2 PG Executor + PR3 Snapshot Executor + PR2 pool-timeout fix. **Pending**: E28.2 PR4 Conformance (closes E28.2 chain), E28.3 Pattern Profile v1, E28.4 Analytics Registry Cohort 1, E28.5 structural cohort 2, E28.6 advanced evidence gate.

/**
 * SVG rendering adapter — owns all visual graph rendering.
 *
 * This module contains the interactive SVG graph with nodes, edges,
 * pan/zoom controls, and accessibility fallbacks. It is
 * layout-agnostic: receives a `LayoutResult` with positions already
 * computed.
 */
...
/**
 * RenderSvgGraph — pure visual renderer with pan/zoom.
 *
 * The component is layout-agnostic: it receives a `LayoutResult`
 * with positions already computed. When the backend
 * `POST /api/diagrams/layout` endpoint lands, swap the mock for
 * an SWR hook that calls it.
 *
 * Interactions:
...
type RenderAdapterInnerProps = RenderAdapter;

function RenderSvgGraphInner({
  layout,
  selectedId = null,
  onSelectObject,
  onViewportChange,

...
use quote::quote;
use syn::{parse_macro_input, ItemFn, ItemStruct, Ident, Type, Expr, Token, LitStr, LitBool};
...
pub fn declare_rule(input: TokenStream) -> TokenStream {
    ...
    let agent_semantics = &rule_input.agent_semantics;
    ...
    // Generate agent_semantics field and method if provided
    let (agent_semantics_field, agent_semantics_default, agent_semantics_method) = if let Some(ref as_input) = agent_semantics {
        let summary = &as_input.summary;
        let fix_playbook = &as_input.fix_playbook;
        let review_questions: Vec<_> = as_input.review_questions.iter()
            .map(|s| quote! { #s.to_string() })
            .collect();
        let semantic_chunks: Vec<_> = as_input.semantic_chunks.iter()
            .map(|s| quote! { #s.to_string() })
            .collect();
        let safe_autofix = as_input.safe_autofix;
        let autofix_guidance = &as_input.autofix_guidance;

        (
            quote! { agent_semantics: Option<crate::rules::types::AgentSemantics> },
            quote! { agent_semantics: Some(crate::rules::types::AgentSemantics::new(#summary.to_string(), #fix_playbook.to_string(), vec![#(#review_questions),*], vec![#(#semantic_chunks),*], #safe_autofix, #autofix_guidance.to_string())) },
            quote! {
                fn agent_semantics(&self) -> Option<&crate::rules::types::AgentSemantics> {
                    self.agent_semantics.as_ref()
                }
            },
        )
    } else {
    ...
    // Generate struct body and default body based on whether params exist
    let (struct_body, default_body) = if params.is_empty() {
        (
            quote! { #agent_semantics_field },
            quote! { #agent_semantics_default },
        )
    } else {
        (
            quote! { #(#field_definitions),* , #agent_semantics_field },
            quote! { #(#field_names: #field_defaults),* , #agent_semantics_default },
        )
    };
...
struct RuleInput {
    ...
    auto_test: bool,
    agent_semantics: Option<AgentSemanticsInput>,
}

/// Parsed agent_semantics block
struct AgentSemanticsInput {
    summary: String,
    fix_playbook: String,
    review_questions: Vec<String>,
    semantic_chunks: Vec<String>,
    safe_autofix: bool,
    autofix_guidance: String,
}

impl syn::parse::Parse for AgentSemanticsInput {
    fn parse(input: syn::parse::ParseStream) -> syn::Result<Self> {
        let content;
        let _brace = syn::braced!(content in input);

        let mut summary = None;
        let mut fix_playbook = None;
        let mut review_questions = None;
        let mut semantic_chunks = None;
        let mut safe_autofix = None;
        let mut autofix_guidance = None;
        ...
        while !content.is_empty() {
            ...
            match key_str.as_str() {
                ...
                "fix_playbook" => {
                    let value: LitStr = content.parse()?;
                    fix_playbook = Some(value.value());
                }
                "review_questions" => {
                    let arr_content;
                    syn::bracketed!(arr_content in content);
                    let mut questions = Vec::new();
                    while !arr_content.is_empty() {
                        let value: LitStr = arr_content.parse()?;
                        questions.push(value.value());
                        if arr_content.peek(Token![,]) {
                            let _comma: Token![,] = arr_content.parse()?;
                        }
                    }
                    review_questions = Some(questions);
                }
                "semantic_chunks" => {
                    let arr_content;
                    syn::bracketed!(arr_content in content);
                    ...
                    semantic_chunks = Some(chunks);
                }
                "safe_autofix" => {
                    let value: LitBool = content.parse()?;
                    safe_autofix = Some(value.value());
                }
                "autofix_guidance" => {
                    let value: LitStr = content.parse()?;
                    autofix_guidance = Some(value.value());
                }
                _ => return Err(syn::Error::new(key.span(), format!("Unknown key in agent_semantics: {}", key_str))),
            }
        ...
        Ok(Self {
            summary: summary.ok_or_else(|| syn::Error::new(input.span(), "Missing 'summary' in agent_semantics"))?,
            fix_playbook: fix_playbook.ok_or_else(|| syn::Error::new(input.span(), "Missing 'fix_playbook' in agent_semantics"))?,
            review_questions: review_questions.ok_or_else(|| syn::Error::new(input.span(), "Missing 'review_questions' in agent_semantics"))?,
            semantic_chunks: semantic_chunks.ok_or_else(|| syn::Error::new(input.span(), "Missing 'semantic_chunks' in agent_semantics"))?,
            safe_autofix: safe_autofix.ok_or_else(|| syn::Error::new(input.span(), "Missing 'safe_autofix' in agent_semantics"))?,
            autofix_guidance: autofix_guidance.ok_or_else(|| syn::Error::new(input.span(), "Missing 'autofix_guidance' in agent_semantics"))?,
        })
    }
}
...
impl syn::parse::Parse for RuleInput {
    fn parse(input: syn::parse::ParseStream) -> syn::Result<Self> {
        ...
        let mut agent_semantics: Option<AgentSemanticsInput> = None;
        ...
        while !input.is_empty() {
            ...
            match key_str.as_str() {
                ...
                "agent_semantics" => {
                    let value: AgentSemanticsInput = input.parse()?;
                    agent_semantics = Some(value);
                }
                _ => return Err(syn::Error::new(key.span(), format!("Unknown key: {}", key_str))),
            }

//! Domain port for persisting cross-service protocol route edges.
//!
//! Cycle e15.5 introduces spec-based ingestion of HTTP / GraphQL /
//! gRPC / tRPC routes. Each ingestion handler parses a spec and saves
...
//!
//! **Naming**: this port was previously called `EdgeEmitter`. It was
//! renamed to `RouteStore` (2026-07-30) because the methods are pure
//! CRUD (`upsert_route`, `save_edge`, `save_many`, `find_*`) rather
//! than fire-and-forget event emission — the `*Store` suffix follows
//! the workspace convention used by `InvestigationStore`,
//! `ViewSpecStore`, `RunLineageStore`, `CallGraphStore`. The
//! `emit_edge` / `emit_many` methods were renamed to `save_edge` /
//! `save_many` for the same reason.
//!
//! ## Idempotency
//!
//! Both `upsert_route` and `save_edge` use ON CONFLICT DO UPDATE
...
/// testable.
///
/// Renamed from `EdgeEmitter` (2026-07-30) — see the module doc for
/// the rationale. The `emit_edge` and `emit_many` methods were
/// renamed to `save_edge` and `save_many` to match the CRUD naming
/// convention.
#[async_trait]
pub trait RouteStore: Send + Sync {
    ...
    async fn save_edge(&self, edge: &ApiRouteEdge) -> ExplorerResult<bool>;

    /// Batch upsert — convenience over `upsert_route` + `save_edge`
...
///
/// Returned by `RouteStore::save_many`. Useful for observability

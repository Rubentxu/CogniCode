//! `PostgresRouteStore` — `RouteStore` port adapter backed by PostgreSQL.
//!
//! Implements all 6 methods of the `RouteStore` port against the
...
//!
//! Both `upsert_route` and `save_edge` use PostgreSQL's
...
#[cfg(feature = "postgres")]
use crate::ports::route_store::{ApiRoute, ApiRouteEdge, BatchStats, RouteStore};
...
/// `PostgresRouteStore` — PG-backed implementation of [`RouteStore`].
///
...
impl RouteStore for PostgresRouteStore {
    ...
    async fn save_edge(&self, edge: &ApiRouteEdge) -> ExplorerResult<bool> {
        ...
        .fetch_one(&self.pool)
        .await
        .map_err(|e| store_err("save_edge", e))?;
    ...
    /// Override the default `save_many` with a transactional batch.
    ///
    ...
    async fn save_many(
        ...
        let mut tx = self
            ...
            .begin()
            .await
            .map_err(|e| store_err("save_many begin", e))?;
        ...
        for route in routes {
            ...
            .fetch_one(&mut *tx)
            .await
            .map_err(|e| store_err("save_many route", e))?;
        ...
        for edge in edges {
            ...
            .fetch_one(&mut *tx)
            .await
            .map_err(|e| store_err("save_many edge", e))?;
        ...
        tx.commit()
            .await
            .map_err(|e| store_err("save_many commit", e))?;
...
mod tests {
    ...
    async fn upsert_route_returns_inserted_then_updated() {
        ...
        let adapter = PostgresRouteStore::from_pool(pool);

...
use cognicode_core::domain::plan::result::{assert_equivalent, ResultSet};
...
fn fixture_abcd_with_direct_d(rev: RevisionId) -> (CallGraph, WorkspaceId) {
    ...
    graph.add_symbol(Symbol::new("D", SymbolKind::Function, Location::new("src/D.rs", 1, 1)));
...
fn fixture_three_paths(rev: RevisionId) -> (CallGraph, WorkspaceId) {
    ...
    let _ = graph.add_dependency_with_provenance(&id_c, &id_d, DependencyType::Calls, ExtractionContext::DirectExtraction);
    let _ = graph.add_dependency_with_provenance(&id_a, &id_d, DependencyType::Calls, ExtractionContext::DirectExtraction);
    let _ = graph.add_dependency_with_provenance(&id_a, &id_b, DependencyType::Calls, ExtractionContext::DirectExtraction); // duplicate A→B
    let _ = graph.add_dependency_with_provenance(&id_b, &id_d, DependencyType::Calls, ExtractionContext::DirectExtraction); // A→B→D path
...
fn path_plan(src: &str, dst: &str, max_hops: u32, limits: PlanLimits) -> GraphPlan {
    GraphPlan::Path {
        src: src.to_string(),
        dst: dst.to_string(),
        quantifier: PathQuantifier { max_hops: Some(max_hops), min_hops: 0 },
...
pg_conformance_test!(
    conformance_unordered_neighbor_sets_match,
    |pool: PgPool| {
        assert_conformant(
            &pool,
            Fixture {
...
pg_conformance_test!(
    conformance_path_sequences_match_in_order,
    |pool: PgPool| {
        assert_conformant(
            &pool,
            Fixture {
...
pg_conformance_test!(
    ...
    |pool: PgPool| {
        ...
        pg_exec.execute(&plan, pin_ok.clone()).expect("valid pin must succeed");
        snap_exec.execute(&plan, pin_ok.clone()).expect("valid pin must succeed");
...
fn loud_failure_panics_with_triple_on_multiset_mismatch() {
    ...
    let mut b = a.clone();
    b.scalars.push(cognicode_core::domain::plan::value::TypedValue::Int(1));

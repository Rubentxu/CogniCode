...
impl GraphBackend {
    ...
    fn find_paths(
        ...
        for (&source_id, source_node) in &nodes {
            if &source_node.subject != source_subject {
                continue;
            }
...
impl DetectorBackend for GraphBackend {
    ...
    fn run(
        ...
        for step in &admitted.definition.steps {
            match step {
                DetectorStep::Flow {
                    ...
                    sink,
                    max_hops,
                } => flow = Some((source, sink, *max_hops)),
        ...
        let Some((source, sink, max_hops)) = flow else {
        ...
        for path in self.find_paths(graph, source, sink, max_hops) {
            // Exclusion is checked on every node of the path.
            let excluded = path.iter().any(|id| {
                node_by_id
                    .get(id)
                    .map(|n| excludes.iter().any(|e| *e == &n.subject))
                    .unwrap_or(false)
            });
            if excluded {
                outcome.diagnostics.push(DetectorDiagnostic {
                    code: "path_excluded".to_string(),
                    message: format!("path from {} to {} excluded by a sanitizer", source, sink),
                });
                continue;
            }

...
impl RunLineageStore for PostgresLineageStore {
    ...
    fn insert(&self, lineage: &RunLineage) -> Result<(), AnalyticsError> {
        ...
        let key_exists: bool = block_on(async move {
        ...
        if key_exists {
            ...
            let existing_params: serde_json::Value = block_on(async move {
                sqlx::query_scalar(
    ...
    fn get(&self, run_id: Uuid) -> Result<RunLineage, AnalyticsError> {
        ...
        let row: Option<AnalyticsRunLineageRow> = block_on(async move {
            sqlx::query_as::<_, AnalyticsRunLineageRow>(
                r#"
    ...
    fn query(
        ...
        let rows: Vec<AnalyticsRunLineageRow> = block_on(async move {

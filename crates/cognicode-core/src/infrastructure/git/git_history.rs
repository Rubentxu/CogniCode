    ...
    match output {
        Ok(output) if output.status.success() => {
            ...
            match trimmed.parse::<i64>() {
                ...
                Err(e) => {
                    tracing::warn!("Failed to parse git timestamp '{}' for {}: {}", trimmed, path_str, e);
                    Ok(None)
                }
            }
        }
        Ok(output) => {
            ...
            tracing::debug!("File {} not tracked by git: {}", path_str, 
                String::from_utf8_lossy(&output.stderr).trim());
            Ok(None)
        }
        Err(e) => {
            // Git command failed (not installed, etc.)
            tracing::warn!("Git command failed for {}: {}. Falling back to mtime.", path_str, e);
            Ok(None)
        }
    }
}
...
mod tests {
    ...
    fn test_get_file_mtime_git_priority() {
        ...
        assert!(source == "git" || source == "mtime", "Source should be git or mtime");
    }
}

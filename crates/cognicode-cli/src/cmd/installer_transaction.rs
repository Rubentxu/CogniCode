...
mod tests {
    ...
    fn custom_home_reviewer_does_not_write_into_ambient_home() {
        ...
        // AssetPoint (RAII) replaces the old point_at(&release) + unpoint()
        // pair — guarantees env cleanup even on assertion panic.
    ...
    fn advance_skips_through_all_stages() {
        ...
        //
        // Use `AssetPoint` (RAII) so a panic or early return restores
        // COGNICODE_ASSET_BASE_URL / COGNICODE_BUNDLE_MANIFEST to their
        // pre-test values. The previous hand-rolled `point_at(&release)`
        // + `unpoint()` pair was easy to forget on the failure path, and
        // any leak would poison the next #[serial] test (regression
        // observed after the 0.98.1 -> 0.99.0 SemVer bump).
...
"#;
    ...
    fn t_l2_extracting_writes_under_versions_layout() {
        ...
        let release = crate::release_test_support::local_release(env!("CARGO_PKG_VERSION"))
            .expect("stage a local release");
        // AssetPoint (RAII) — replaces point_at(&release) (no unpoint) which
        // leaked env state to the next #[serial] test.
        let _point = crate::release_test_support::AssetPoint::new(&release);

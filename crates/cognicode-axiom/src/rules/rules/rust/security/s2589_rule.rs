...
declare_rule! {
    ...
    check: => {
        let mut issues = Vec::new();
        // Uses fancy_regex for lookahead support (?![0-9])
        let re = match fancy_regex::Regex::new(r#""([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])"(?![0-9])"#) {
            Ok(re) => re,
        ...
        for (idx, line) in ctx.source.lines().enumerate() {
            ...
            if let Ok(Some(m)) = re.find(trimmed) {
                issues.push(Issue::new(
                    "S1313",

#!/usr/bin/env bash
set -uo pipefail
# E2E Test: rename function via safe_refactor MCP tool
# Validates: MCP protocol, tool response correctness, change preview accuracy
# Env: BINARY, RESULTS_DIR, SCENARIO_NAME

SCENARIO_NAME="${SCENARIO_NAME:-rename_function}"
...
echo "Running rename_function scenarios..."

# Create temp fixture for reliable testing
tmpdir=$(mktemp -d)
mkdir -p "$tmpdir/src"
...
# Step 1: Call safe_refactor to get rename preview
mcp_response=$("$MCP_CLIENT" \
    --workspace "$tmpdir" \
    --method "tools/call" \
    --params "{\"name\":\"safe_refactor\",\"arguments\":{\"action\":\"rename\",\"target\":\"add\",\"params\":{\"new_name\":\"sum\",\"file_path\":\"$tmpdir/src/lib.rs\"}}}" \
...
if [ "$mcp_ok" = "true" ]; then
    ...
    if [ -n "$tool_text" ]; then
        # Check if refactor preview was successful
        ...
        if [ "$refactor_success" = "true" ] && [ "$change_count" -ge 1 ]; then
            success=true
            files_modified=1
            build_passed=true  # Preview was valid
            test_passed=true   # Protocol correctness verified
        else
            error_msg=$(echo "$tool_text" | jq -r '.error_message // "unknown"' 2>/dev/null)
            error_message="Refactor preview failed: $error_msg (changes=$change_count, valid=$validation_valid)"
        fi
    else
